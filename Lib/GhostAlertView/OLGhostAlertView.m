...
{
    self = [self initWithTitle:title message:message timeout:3 dismissible:YES];
...
{
    self = [self initWithTitle:title message:nil timeout:3 dismissible:YES];
...
- (void)layoutSubviews
{
    ...
    if (self.messageLabel) {
        self.messageLabel.frame = CGRectMake(self.messageLabel.frame.origin.x, titleSize.height + floorf(VERTICAL_PADDING * 1.5) - 3, totalLabelWidth, messageSize.height);
    }
}
